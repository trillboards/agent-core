 ...
 */
data class AudienceMetricsPayload(
    val screenId: String?,
    val fingerprint: String,
    val timestamp: Long,
    val intervalMs: Long,  // How long this measurement covers
    val viewerCount: Int,
    val personCount: Int,  // TFLite person detection (PoC)
    val peakViewerCount: Int,
    val attentionScore: Float,
    val demographics: Demographics,
    val dwellTime: DwellTimeMetrics,
    val environment: EnvironmentMetrics
) {
    fun toMap(): Map<String, Any?> = mapOf(
        ...
        "peakViewerCount" to peakViewerCount,
        "attentionScore" to attentionScore,
        "demographics" to mapOf(
            "ageRanges" to demographics.ageRanges,
            "genderDistribution" to demographics.genderEstimates  // Changed from "genders" to match API contract
        ),
        "dwellTime" to mapOf(
            "avg" to dwellTime.averageSeconds,
            "max" to dwellTime.maxSeconds,
            "min" to dwellTime.minSeconds,
            "total" to dwellTime.totalViewerSeconds
        ),
        "environment" to mapOf(
            "ambientLight" to environment.ambientLightLux,
            "temperature" to environment.deviceTemperatureC,
            "orientation" to environment.orientation
        )
    )
}
